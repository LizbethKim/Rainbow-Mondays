...
class Controller {
    ...
    public function getInfoAction(){
      ...
      if ($cat != 0 && $level == 0){
        ...
        $result2 = array_filter($result2);
        if (empty($result2)){
          $result2 = $daoJobs->query("SELECT count(j.id), max(batchid), type FROM jobs j JOIN districts d ON j.locationId = d.id JOIN categories c on j.categoryId = c.id WHERE d.id = $currBest AND c.parentCategoryId = $cat GROUP BY type");
        }
      } else {
        $result2 = $daoJobs->query("SELECT count(j.id), max(batchid), type FROM jobs j JOIN districts d ON j.locationId = d.id WHERE d.id = $currBest GROUP BY type");
      }
      ...
      if ($cat == 0 && $level ==0){
        ...
        $averageAge = $daoJobs->query("SELECT avg(listedTime) from jobs j JOIN districts d ON j.locationId = d.id WHERE d.id = $currBest");
      } else {
        $averageAge = $daoJobs->query("SELECT avg(listedTime) from jobs j JOIN districts d ON j.locationId = d.id WHERE d.id = $currBest AND j.categoryId = $cat");
        if (empty($averageAge[0])){
          $averageAge = $daoJobs->query("SELECT avg(listedTime) from jobs j JOIN districts d ON j.locationId = d.id JOIN categories c on j.categoryId = c.id WHERE d.id = $currBest AND c.parentCategoryId = $cat");
        }
      }
      $return = [];
